...
export class AudioTranscriber {
  ...
  async transcribe(audioPath: string): Promise<TranscriptionResult> {
    ...
    try {
      ...
      console.log('Processing audio with VAD v4 + Whisper...');
  ...
  private createRecognizer() {
    const config = {
      ...
      modelConfig: {
        whisper: {
          encoder: this.config.whisper.encoder,
          decoder: this.config.whisper.decoder,
        },
        tokens: this.config.whisper.tokens,
        numThreads: this.config.whisper.numThreads,
        provider: this.config.whisper.provider,
        debug: this.config.whisper.debug,
      },
    };
