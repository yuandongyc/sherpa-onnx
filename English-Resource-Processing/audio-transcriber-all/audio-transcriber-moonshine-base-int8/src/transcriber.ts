...
export class AudioTranscriber {
  ...
  async transcribe(audioPath: string): Promise<TranscriptionResult> {
    ...
    try {
      ...
      console.log('Processing audio with VAD v4 + Moonshine (Base int8)...');
  ...
  private createRecognizer() {
    const config = {
      ...
      modelConfig: {
        moonshine: {
          preprocessor: this.config.moonshine.preprocessor,
          encoder: this.config.moonshine.encoder,
          uncachedDecoder: this.config.moonshine.uncachedDecoder,
          cachedDecoder: this.config.moonshine.cachedDecoder,
        },
        tokens: this.config.moonshine.tokens,
        numThreads: 2,
        provider: 'cpu',
        debug: 0,
      },
