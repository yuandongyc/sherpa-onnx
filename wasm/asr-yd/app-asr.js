...
const modelFolderInput = document.getElementById('modelFolder');
...
function processAudioFile(arrayBuffer) {
  ...
  audioCtx.decodeAudioData(arrayBuffer, function(audioBuffer) {
    ...
    // Downsample to 16kHz
    const downsampledSamples = downsampleBuffer(samples, expectedSampleRate);
    ...
    for (let i = 0; i < downsampledSamples.length; i += chunkSize) {
      const chunk = downsampledSamples.subarray(i, Math.min(i + chunkSize, downsampledSamples.length));
    ...
    // Check if it's a paraformer model
    if (recognizer.config.modelConfig.paraformer.encoder != '') {
      let tailPaddings = new Float32Array(expectedSampleRate);
      recognizer_stream.acceptWaveform(expectedSampleRate, tailPaddings);
      while (recognizer.isReady(recognizer_stream)) {
    ...
    alert('Error decoding audio file. Please try again with a different file.');
  });
};
